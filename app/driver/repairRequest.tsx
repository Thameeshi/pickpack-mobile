...
export default function RepairRequestScreen() {
  ...
  const handleSubmit = async () => {
    if (!repairType) {
      Alert.alert('Error', 'Please select a repair type');
      return;
    }
    if (!description.trim()) {
        ...
        <View style={styles.typeGrid}>
          {REPAIR_TYPES.map(rt => (
            <TouchableOpacity
              ...
              style={[
                styles.typeChip,
                repairType === rt.key && styles.typeChipActive,
              ]}
              onPress={() => setRepairType(rt.key)}
            >
              <Text style={styles.typeChipIcon}>{rt.icon}</Text>
              <Text
                style={[
                  styles.typeChipText,
                  repairType === rt.key && styles.typeChipTextActive,
                ]}
                numberOfLines={1}
              >
...
const styles = StyleSheet.create({
  ...
  typeChip: {
    ...
    borderRadius: RADIUS.FULL, borderWidth: 1.5, borderColor: COLORS.GRAY_200,
  ...
  typeChipText: { fontSize: FONT_SIZES.SM, fontWeight: '600', color: COLORS.GRAY_600 },
